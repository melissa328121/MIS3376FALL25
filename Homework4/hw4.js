...
//code for the cookie to remember any information inputted on the form
// create a cookie
...
// The cookie logic only the first name is remembered//

// check if a returning user exists
const savedFirstName = getCookie("firstName");

// if cookie exists then show the “Welcome back”
if (savedFirstName !== "") {
  document.getElementById("Welcome1").textContent =
    "Welcome back, " + savedFirstName + ".";

  // clickable 
  document.getElementById("Welcome2").innerHTML =
    `<a href="#" id="new-user">Not ${savedFirstName}? Start a new form.</a>`;

  // clear only the first name cookie when they click new user
...
// if cookie does not exist then new user greeting
} else {
  document.getElementById("Welcome1").textContent = "Welcome New User!";
  document.getElementById("Welcome2").textContent = "";
}


// Will save the first name to the cookie when the user type the 'Remember Me'//

// on first name input then update cookie for 2 days
...
// =====================================================
// LOCAL STORAGE SYSTEM (HOMEWORK 4 REQUIREMENT)
// Save ALL non-secure fields (everything except first name)
// Load them back when user returns and cookie matches
// =====================================================

// list of fields to store in local storage
var localFields = [
  "minitial",
  "lname",
  "dob",
  ...
  "range",
  "username"
  // fname is NOT stored here → it goes in COOKIES only
];

// =====================================================
// SAVE TO LOCAL STORAGE when user types or leaves field
// =====================================================
localFields.forEach(function(fieldId) {
  var element = document.getElementById(fieldId);

  if (!element) return; // safeguard

  // save on any input/blur change
...
// =====================================================
// SPECIAL CASES: RADIO BUTTONS + CHECKBOXES
// (must be handled manually because their value is not
...
document.querySelectorAll("input[name='insurance']").forEach(function(r) {
  r.addEventListener("change", function() {
    localStorage.setItem("insurance", r.value);
  });
});
...
document.querySelectorAll("input[name='vaccinated']").forEach(function(r) {
  r.addEventListener("change", function() {
    localStorage.setItem("vaccinated", r.value);
  });
});

// condition checkboxes — save array
document.querySelectorAll("input[name='condition']").forEach(function(ch) {
  ch.addEventListener("change", function() {
    const checkedValues = Array.from(
      document.querySelectorAll("input[name='condition']:checked")
    ).map(c => c.value);

    localStorage.setItem("conditionList", JSON.stringify(checkedValues));
  });
});


// This is the localstorage code for the form//
document.addEventListener("DOMContentLoaded", function() {

  // only load if a first name cookie exists
  const storedFirst = getCookie("firstName");

  if (storedFirst !== "") {

    // load text inputs and the select fields
    localFields.forEach(function(fieldId) {
      var value = localStorage.getItem(fieldId);
      if (value !== null) {
    ...
    // load insurance radio buttons
    const savedInsurance = localStorage.getItem("insurance");
    if (savedInsurance) {
      let radio = document.querySelector(`input[name='insurance'][value='${savedInsurance}']`);
      if (radio) radio.checked = true;
    }

    // load vaccinated radio buttons
    const savedVaccinated = localStorage.getItem("vaccinated");
    if (savedVaccinated) {
      let radio = document.querySelector(`input[name='vaccinated'][value='${savedVaccinated}']`);
      if (radio) radio.checked = true;
    }

    // load condition checkboxes
    const savedConditions = JSON.parse(localStorage.getItem("conditionList") || "[]");
    savedConditions.forEach(function(condValue) {
      let checkbox = document.querySelector(`input[name='condition'][value='${condValue}']`);
...
// the remember me checkbox if this part unchecked then clear all the localStorage and cookie//
document.getElementById("rememberMe").addEventListener("change", function() {

  if (!this.checked) {

    // delete first name cookie and clear all localStorage
    deleteCookie("firstName");
    localStorage.clear();

  } else {

    // if checked again then save fname to cookie
    const f = document.getElementById("fname").value.trim();
    if (f !== "") {
      setCookie("firstName", f, 2); // 2 days
    }
  }

});

// Fetch api part of the js for the state drop list //
